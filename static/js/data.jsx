...
function parseLogLines(lines) {
  ...
  return (lines || []).map((line, i) => {
    ...
    // strip leading journalctl timestamp prefix if present
    const msg = line.replace(/^[A-Z][a-z]{2}\s+\d+\s+[\d:]+\s+\S+\s+\S+:\s*/, '').trim() || line;
    return { t: now - (lines.length - i) * 2000, level, msg };
  });
}
